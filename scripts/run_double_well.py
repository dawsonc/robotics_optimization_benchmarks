...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Run a suite of experiments on the double well example."
    )
    parser.add_argument(
        "--results_dir",
        type=str,
        default="./results/double_well",
    ...
    if not args.load_data:
        for dimension in [1, 10, 100, 1000]:
            # Create an experiment suite for the double_well
            experiment_suite = experiment_suite_factory.create_experiment_suite(
                name=f"double_well_{dimension}",
                description="Compare optimization and inference methods on the double_well env.",
                seeds=list(range(10)),
